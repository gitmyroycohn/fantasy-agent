        ...
        if atype == "matchup_summary":
            ...
            if signals:
                ...
                if sells:
                    ...
                    for s in sells[:4]:
                        ...
                        print(f"      {s['reason']}")
                if buys:
                    print(f"  BUY LOW ({len(buys)}) -- underperforming on other teams:")
                    for s in buys[:4]:
                        pos       = "/".join(s.get("positions", []))
                        conf      = s.get("confidence", "")
                        owner     = s.get("_owner", "")
                        owner_str = f" owned by: {owner}" if owner else ""
                        print(f"   ~ {s['name']} ({s['team']}) [{pos}] [{conf}]{owner_str}")
                        print(f"      {s['reason']}")
            ...
            if roster_hits:
                ...
                for t in roster_hits:
                    icon = "\U0001f691" if t["type"] == "placed" else ("\u2705" if t["type"] == "activated" else "\U0001f504")
                    date_str = t.get("date", "")
                    try:
            ...
            if other_txns:
                ...
                if placed:
                    print(f"  \U0001f691 Placed ({len(placed)}):", ", ".join(
